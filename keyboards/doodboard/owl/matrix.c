...
#ifdef SCAN_MATRIX_IN_LOGICAL_ORDER
    uint8_t mask = row;
#else
...
static bool read_cols_on_row(matrix_row_t current_matrix[], uint8_t active_row){
#ifdef SCAN_MATRIX_IN_LOGICAL_ORDER
    uint8_t current_row = row_masks[active_row];
#else
    uint8_t current_row = active_row;
#endif
